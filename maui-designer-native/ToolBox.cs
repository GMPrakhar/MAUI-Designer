...
namespace MAUIDesigner
{
    internal class ToolBox
    {
        ...
        static ToolBox()
        {
            IconMapping = IconMappingLoader.Load();
        }
        ...
        private static string GetIconForElement(string elementName)
        {
            ...
            return IconMapping.TryGetValue(elementName, out var icon) && !string.IsNullOrEmpty(icon)
                ? icon
                : IconMapping.TryGetValue("Default", out var fallback) ? fallback : "\ue724";
        }
        ...
        internal static View GetViewForPropertyType(View view, PropertyInfo property, object value)
        {
            ...
            else if (property.PropertyType == typeof(ColumnDefinitionCollection))
            {
                ...
                editor.TextChanged += (s, e) =>
                {
                    try
                    {
                        ...
                        var columnDefinitions = text.Split(',').Select(x => new ColumnDefinition(ParseGridLength(x)));
            ...
            else if (property.PropertyType == typeof(RowDefinitionCollection))
            {
                ...
                editor.TextChanged += (s, e) =>
                {
                    ...
                    {
                        var text = (s as Entry)!.Text;
                        var RowDefinitions = text.Split(',').Select(x => new RowDefinition(ParseGridLength(x)));
            ...
            else if (property.PropertyType == typeof(Color) || property.PropertyType == typeof(Thickness))
            {
                ...
                void TextChanged(object? sender, TextChangedEventArgs e)
                {
                    try
                    {
                        var valueA = double.Parse(red.Text, CultureInfo.InvariantCulture);
                        var valueB = double.Parse(green.Text, CultureInfo.InvariantCulture);
                        var valueC = double.Parse(blue.Text, CultureInfo.InvariantCulture);
                        var valueD = double.Parse(alpha.Text, CultureInfo.InvariantCulture);
                        if (property.PropertyType == typeof(Color))
                        {
                            var color = Color.FromRgba(
                                (byte)Math.Clamp(valueA, 0, 255),
                                (byte)Math.Clamp(valueB, 0, 255),
                                (byte)Math.Clamp(valueC, 0, 255),
                                (byte)Math.Clamp(valueD, 0, 255));
        ...
        internal static GridLength ParseGridLength(string token)
        {
            token = (token ?? string.Empty).Trim();
            if (token.Length == 0 || token.Equals("Auto", StringComparison.OrdinalIgnoreCase))
            {
            ...
            {
                var factor = token[..^1];
                if (double.TryParse(factor, NumberStyles.Float, CultureInfo.InvariantCulture, out var star))
                {
            ...
            {
                return new GridLength(absolute, GridUnitType.Absolute);
            }
