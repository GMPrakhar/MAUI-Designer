...
- **Extension tests**: `dotnet test extension/MauiDesigner.Core.sln` -- xUnit suite for the Visual Studio extension's core, plus `MauiDesigner.Vsix.CompileCheck`, which compiles the extension sources against the real VS SDK reference assemblies, and `RegistrationMetadataTests`, which reads the compiled registration attributes with `MetadataLoadContext`. All run on any OS; only *packaging* the VSIX needs Windows. Keep the VSIX UI free of XAML markup files so it stays compilable on Linux.
...
- **CI/CD**: `.github/workflows/ci.yml` builds and runs unit + e2e tests plus the extension's `dotnet test`; `.github/workflows/deploy-pages.yml` publishes the static build to GitHub Pages on pushes to `main`; `.github/workflows/release-vsix.yml` packages the VSIX on Windows (the only step that needs Windows), then installs it into a real Visual Studio on both `windows-2022` (VS 2022) and `windows-latest` (VS 2026) and verifies the registration VS stored in its private registry hive, then publishes it as a pre-release asset called `MauiDesigner.vsix` when a `vsix-v*` tag is pushed. Reading that hive has two traps: open it with `RegLoadAppKey` (not `reg load`, which fails with *Access is denied* while VS service hosts hold the file), and look under `Software\Microsoft\VisualStudio\<version>_<instance>_Config` -- VS merges pkgdef registrations into the `_Config` key, while the bare `<version>_<instance>` key holds user settings, so reading it returns a null that is easily mistaken for a rejected registration. Do not try to drive a full `devenv` start to force the merge; it sits on first-run UI and never returns. `devenv /updateconfiguration` is the documented mechanism and exits on its own. Publishing is gated on the install check, so an uninstallable VSIX cannot ship. Note that `VSIXInstaller.exe` exits `0` even when it decides the extension does not apply to that Visual Studio, so the manifest's `InstallationTarget` range (currently `[17.0,19.0)`, covering VS 2022 and 2026) has to be checked by looking for the deployed files, not the exit code. The site's download link resolves to `releases/download/vsix-latest/MauiDesigner.vsix`, so publishing a new release updates it without redeploying the site.
