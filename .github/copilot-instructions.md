...
- **CI/CD**: `.github/workflows/ci.yml` builds and runs unit + e2e tests plus the extension's `dotnet test`; `.github/workflows/deploy-pages.yml` publishes the static build to GitHub Pages on pushes to `main`; `.github/workflows/release-vsix.yml` packages the VSIX on Windows (the only step that needs Windows), installs it into the real Visual Studio Enterprise 2022 that ships on the `windows-latest` image and verifies the registration VS stored in its private registry hive, then publishes it as a pre-release asset called `MauiDesigner.vsix` when a `vsix-v*` tag is pushed. Publishing is gated on the install check, so an uninstallable VSIX cannot ship. The site's download link resolves to `releases/download/vsix-latest/MauiDesigner.vsix`, so publishing a new release updates it without redeploying the site.
